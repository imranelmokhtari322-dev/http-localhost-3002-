...
const PosterCard: React.FC<{ item: MediaItem }> = ({ item }) => (
  <div className="group relative flex-shrink-0 w-[150px] sm:w-[170px] h-[230px] sm:h-[260px] rounded-2xl overflow-hidden cursor-pointer hover:scale-[1.04] transition-transform duration-300 shadow-xl">
    ...
    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
      <div className="p-3 rounded-full bg-sky-400 text-white scale-75 group-hover:scale-100 transition-transform duration-300 shadow-lg">
...
const SectionRow: React.FC<{ label: string; title: string; badge: string; children: ReactNode }> = ({ label, title, badge, children }) => (
  <div className="mb-12">
    <div className="flex items-center justify-between mb-5">
      <div>
        <span className="text-[10px] uppercase font-mono text-sky-600 font-bold tracking-widest block mb-1">{label}</span>
        <h2 className="text-xl sm:text-2xl font-extrabold font-display tracking-tight text-gray-900">{title}</h2>
      </div>
      <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest hidden sm:block">{badge}</span>
    </div>
    {children}
  </div>
);
...
export default function SportsAndEntertainment() {
  return (
    <section className="bg-dark-bg text-gray-900 py-14 border-b border-sky-200 overflow-hidden" id="sports-entertainment-section">
