...
export default function SetupWizard() {
  ...
  return (
    <section className="bg-dark-bg text-gray-900 py-16 border-b border-sky-200" id="setup-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-xs uppercase font-mono text-sky-600 font-bold tracking-widest bg-sky-100 border border-sky-200 px-3 py-1 rounded-full mb-3 inline-block">STAP-VOOR-STAP INSTALLATIE</span>
          <h2 className="text-3xl sm:text-4xl font-extrabold font-display tracking-tight text-gray-900 mb-4">Eenvoudige installatie op elk apparaat</h2>
          <p className="text-gray-500 text-sm sm:text-base">Selecteer je apparaat en volg de simpele 4-stappen handleiding.</p>
        </div>
        <div className="flex flex-wrap justify-center gap-2.5 mb-10">
          {devices.map((dev) => (
            <button key={dev.id} onClick={() => setSelectedDevice(dev.id)}
              className={`px-4 py-3 rounded-xl text-xs font-bold tracking-wide transition-all gap-2 flex items-center cursor-pointer ${selectedDevice === dev.id ? "bg-gray-900 text-white shadow-md scale-[1.02]" : "bg-sky-100 border border-sky-200 text-gray-700 hover:bg-sky-200 hover:text-gray-900"}`}>
        ...
        <div className="flex flex-col gap-4 text-left">
          {instructions[selectedDevice].map((inst) => (
            <div key={inst.step} className="p-5 rounded-2xl border border-sky-200 bg-dark-card flex gap-4 items-start shadow-sm">
              <div className="w-8 h-8 rounded-lg bg-sky-100 border border-sky-300 text-sky-700 flex items-center justify-center font-bold text-sm shrink-0 font-mono">{inst.step}</div>
              <div>
                <h4 className="text-sm font-bold text-gray-900 mb-1 uppercase font-mono tracking-wider">Stap {inst.step}</h4>
                <p className="text-xs text-gray-500 leading-relaxed">{inst.text}</p>
              </div>
            </div>
          ))}
        </div>
